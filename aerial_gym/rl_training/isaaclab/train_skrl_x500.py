"""
Train PositionSetpointX500Env with skrl PPO.

x500 quadrotor (1.656 kg, 20 N max thrust per motor).
Same 15-dim observation as end-to-end (rotation 6D, world-frame linvel).

Usage:
    /home/cow_server01/pg-dev/isaacsim/python.sh \
        aerial_gym/rl_training/isaaclab/train_skrl_x500.py \
        --num_envs 1024 \
        --headless \
        --timesteps 1000000

Resume training:
    ... train_skrl_x500.py --checkpoint logs/skrl/.../agent_*.pt
...
parser = argparse.ArgumentParser(description="Train x500 position setpoint with skrl PPO")
...
from aerial_gym.envs.direct.position_setpoint_x500_env import (
    PositionSetpointX500Env,
    PositionSetpointX500EnvCfg,
)
...
def main():
    ...
    env = PositionSetpointX500Env(cfg=cfg, render_mode=None)
    ...
    cfg_path = os.path.join(
        os.path.dirname(__file__), "agents", "x500_skrl_ppo.yaml"
    )
